...
### 👉 https://pittua.github.io/Monsters/

インストール不要。**PC でもスマートフォンでも**ブラウザで開くだけ。

- **ローカル2人 / CPU対戦 / 練習** … その場ですぐ遊べる（サーバー不要）
- **オンライン2人** … 一方が「ルームを作成」して4桁コードを発行 → もう一方が同じ URL を開いて「ルームに参加」でコード入力。2台のスマホ、スマホ＋PC など離れた相手と対戦できる

> オンライン対戦は無料ホスティング（Render）上のサーバーに接続する。しばらくアクセスがないとスリープしているため、最初の接続だけ起動に数十秒かかることがある（「サーバーを起こしています…」と表示され自動でつながる）。

ゲームの流れ: タイトル → 対戦形態 → ルールモード → モンスター作成 → 先攻決定 → バトル → 結果。ルールの詳細は [DESIGN.md](./DESIGN.md) を参照。

## 自分で動かす（開発）

ビルド不要の Vanilla JS（ES モジュール）＋ Node/ws の結合サーバー。サーバーを1つ起動すれば、静的配信・WebSocket・`/health` が同じプロセスで動く:

```bash
npm install          # 初回のみ（ws を取得）
npm start            # node server/server.js
# → http://localhost:8080 を2つのタブ/端末で開いてオンライン対戦も確認できる
```

ローカル2人・CPU・練習だけなら任意の静的サーバ（例: `npx serve .`）でも遊べる（ES モジュールのため `file://` 直開きは不可）。
...
## 自分で公開する（フォーク向け・一度きり・以降コマンド不要）

フォークして自分のインスタンスを公開したい場合の手順。一度デプロイすれば、**サーバー起動コマンドを毎回打つ必要はなく、ページURLを開くだけ**でローカル/CPU/オンライン対戦まで遊べる。2通りの構成がある（上の「いますぐ遊ぶ」は構成Aで運用）。
