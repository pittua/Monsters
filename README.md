...
結合サーバーなので Render の **Web サービス1つ**で完結する。

- Build Command: `npm install`
- Start Command: `npm start`（`PORT` 環境変数を自動使用）
- keep-alive は入れない（スリープ許容）。初回接続時はコールドスタートで30〜50秒かかり得るため、クライアントは「サーバーを起こしています…」を表示しつつ最大75秒リトライする
- 死活確認用に `GET /health` を用意（常時 ping はしない）
- フロントを別オリジンに分離したい場合は、クライアント側で `?ws=wss://...` クエリか `window.WS_URL` で WS 接続先を上書き可能
